...
`AGENTS.md` and `CLAUDE.md` are byte-identical files, generated by
concatenating two sources:

1. `AGENTS-Source.md` from the project where `c.ps1` was launched — the
   local, project-specific part. Optional; if missing, only the suffix is
   emitted.
2. `AGENTS-Suffix.md` from the AgentCli repo — shared boilerplate appended
   to every project. Required.

Both filenames are produced because different CLIs look for different
filenames (Claude reads `CLAUDE.md`, others read `AGENTS.md`).
